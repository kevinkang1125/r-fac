...
def train_off_policy_multi_agent(env, mixer,agents, replay_buffers, num_episodes, minimal_size, batch_size,rho,iter):
    return_multi_list = []
    td_list = []
    epoch_num = 10
    for i in range(epoch_num):
        with tqdm(total=int(num_episodes / epoch_num), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes / epoch_num)):
                episode_return = each_sampling(env, agents, replay_buffers,rho)
                return_multi_list.append(episode_return)
                if replay_buffers[0].size() > minimal_size:
                    td_errors = central_train_off_policy(mixer,replay_buffers,batch_size,rho,agents,iter)
                    td_list.extend(td_errors)
                if (i_episode + 1) % 10 == 0:
...
def central_train_off_policy(mixer,replay_buffers,batch_size,rho,agents,iter):
    alive_list = []
    td_list = []
    transition_dicts = []
