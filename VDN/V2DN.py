...
class VDN:
    def __init__(self,agents,gamma_2,agent_num,batch_size):
        self.agents = agents
        self.gamma = gamma_2
        self.agent_num = agent_num
        self.batch_size = batch_size
    def learn(self, current_q_values, next_max_q_values,rewards, alive_index,transition_list):
        # Get Q-values for the current state and action for all agents
        joint_current = torch.zeros(self.batch_size,1)
        joint_next = torch.zeros(self.batch_size,1)
        team_reward = torch.zeros(self.batch_size,1)
